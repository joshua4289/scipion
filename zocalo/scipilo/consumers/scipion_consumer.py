...
class ScipionMain(CommonService):
    '''A zocalo service for running Scipion'''


    # Human readable service name
    _service_name = "Scipion_Main"
    ...
    def run_scipion(self, rw, header, message):
       ...
       # get the parameters
        ...
        rw.send([])
        self.log.info('Processing session procedure complete on {0}'.format(str(gda2_workspace_dir)))
    ...
    def create_json_file_from_template(self, template_filename, output_filename, session, project_path):
        ...
        config_file = json.load(open(template_filename))
        ...
        for i in range(len(config_file)):
            ...
            # if prot['object.className'] == 'ProtRelionExtractParticles':
            #
            #     bg_diameter = calculate_relion_bgradius(boxSize)
            #     prot['backDiameter'] = bg_diameter
