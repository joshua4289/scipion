...
# Where to save the new custimized workflow (config/scipionbox.conf)
...
DIAMETER_OF_PARTICLE='diameterOfParticle'
GAUTOMATCH_MIN_DIST='minDist'


vars2Use = [DOSE_PER_FRAME, MAGNIFICATION, NUMBER_OF_FRAMES, SAMPLING_RATE, WINDOW_SIZE,GAUTOMATCH_MIN_DIST,FILES_PATH]
...
LABELS = {
    ...
    MAGNIFICATION: "Magnification",
    SAMPLING_RATE: "Pixel Size",
    WINDOW_SIZE: " CTF Window Size",
    DIAMETER_OF_PARTICLE:"Diameter of Particle",
    GAUTOMATCH_MIN_DIST:"Gautomatch Minimum Distance",
...
class BoxWizardView(tk.Frame):
    ...
    def _fillContent(self, frame):
        ...
        def _addCheckPair(key, r, lf, col=1, value=0):
            t = LABELS.get(key, key)
            var = tk.IntVar()
            var.set(0)
        ...
        _addCheckPair (PHASEPLATE, 7, labelFrame2, value=0)
        _addPair (DIAMETER_OF_PARTICLE, 8, labelFrame2, value=250)
        _addPair (GAUTOMATCH_MIN_DIST,9,labelFrame2) 
        _addPair(FILES_PATH, 10, labelFrame2)

        frame.columnconfigure(0, weight=1)

        self._onInputChange()

    def _getConfValue(self, key, default=0):
    ...
    def _createTemplate(self):
        ...
        try:
            ...
            #this is a key set in the original .json 

            replacementDict['findPhaseShift'] = self._getValue(PHASEPLATE) or 0
            replacementDict['particleSize'] = self._getValue(DIAMETER_OF_PARTICLE)
            


            replacementDict['minDist'] = self._getValue(GAUTOMATCH_MIN_DIST)

            if replacementDict['minDist'] =="":
                #print type(replacementDict['particleSize'])

                replacementDict['minDist'] = str(0.9 * float(replacementDict['particleSize']))
            ...
            #print "**************************************************"
            #print "The Value Of PHASEPLATE is " %(str(replacementDict['findPhaseShift']))
            #print "**************************************************"
